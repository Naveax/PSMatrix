...
def _direct_database_candidate(path: Path, *, label: str, must_exist: bool) -> Path:
    candidate = _reject_indirect_components(path, label=label)
    _direct_existing_directory(candidate.parent, label=f"{label} parent")
    try:
        info = candidate.lstat()
    except FileNotFoundError:
        if must_exist:
            raise RemoteProtocolError(f"{label} file not found: {candidate}")
        return candidate
    except OSError as exc:
...
class ReplayGuard:
    def __init__(self, path: Path):
        candidate = _reject_indirect_components(path, label="Replay guard database")
