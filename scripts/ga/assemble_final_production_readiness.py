...
def assemble(*, contract_path: Path, receipts_dir: Path, output: Path) -> dict[str, Any]:
    ...
    if set(receipts) != set(expected_names):
        raise ProductionReadinessError(f"readiness receipt environment set mismatch: expected={sorted(expected_names)} actual={sorted(receipts)}")
    ...
    for expected in contract["environments"]:
        ...
        if set(by_key) != set(required) or len(by_key) != len(checks):
        ...
        for source, check_name in required:
            ...
            if source == "var" and check_name in path_vars:
                if item.get("path_exists") is not True:
                    missing_paths.append(check_name)
            elif "path_exists" in item:
