...
def import_release(
    *,
    ...
    target = _require_clean_destination(
        destination if destination is not None else ga / "media" / "release" / version
    )
    ...
    report = {
        ...
        "ga_eligible": False,
        "next_required": [
            "Run media inventory with an explicit isolated SearchRoot that includes this imported RC release root and only reviewed external-media roots.",
            "Canonicalize that isolated inventory and require release-manifest closure READY before media-manifest materialization.",
            "Do not include the broad Downloads root containing historical RC2 release evidence in the active RC3 authority inventory.",
        ],
    }
    report_path = ga / "windows-authority-protected-release-import.json"
