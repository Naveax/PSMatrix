...
_CONTRACT = ROOT / "ga-packs" / "03-authoritative-windows" / "final-release-source-promotion-contract.json"
...
def _read_contract() -> dict[str, Any]:
    try:
        value = json.loads(_CONTRACT.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RuntimeError("Final release source-promotion contract is unreadable") from exc
    if not isinstance(value, dict):
        raise RuntimeError("Final release source-promotion contract root must be an object")
    if value.get("schema") != 1 or value.get("kind") != "psmatrix.windows-authority-final-release-source-promotion-contract":
        raise RuntimeError("Final release source-promotion contract identity mismatch")
    if value.get("pack") != "03-authoritative-windows":
        raise RuntimeError("Final release source-promotion contract pack mismatch")
    if value.get("from_version") != "2.0.0rc4" or value.get("to_version") != _FINAL_VERSION:
        raise RuntimeError("Final release source-promotion version boundary mismatch")
    anchor = str(value.get("rc4_hardened_source_head") or "").lower()
    if not _SHA40.fullmatch(anchor):
        raise RuntimeError("Final release source-promotion RC4 anchor is invalid")
    promotion = value.get("promotion") if isinstance(value.get("promotion"), dict) else {}
    required_true = (
        "separate_final_commit_required",
        "exact_rc4_anchor_must_be_ancestor",
        "final_lock_must_be_absent_in_preparation",
        "final_public_authority_must_be_absent_in_preparation",
    )
    for field in required_true:
        if promotion.get(field) is not True:
            raise RuntimeError(f"Final release promotion contract field must be true: {field}")
    if promotion.get("rc4_evidence_may_be_relabelled_as_final") is not False:
        raise RuntimeError("RC4 evidence relabelling must remain forbidden")
    return value
...
def _project_version() -> str:
    ...
    if version != _FINAL_VERSION or __version__ != _FINAL_VERSION:
        raise RuntimeError(f"Final release identity mismatch: pyproject={version}, package={__version__}")
    return version


def _require_promotion_boundary(head: str) -> dict[str, Any]:
    contract = _read_contract()
    anchor = str(contract["rc4_hardened_source_head"]).lower()
    if head == anchor:
        raise RuntimeError("Final 2.0.0 release requires a distinct commit after the hardened RC4 source head")
    ...
    if ancestor.returncode != 0:
        raise RuntimeError(
            "Exact hardened RC4 source head is not an ancestor of the final release commit; "
            "fetch full history and use the reviewed final promotion lineage"
        )

    changed = tuple(
        line.strip().replace("\\", "/")
        for line in _git_text("diff", "--name-only", f"{anchor}..{head}").splitlines()
        if line.strip()
    )
    promotion = contract["promotion"]
    allowed = {str(item).replace("\\", "/") for item in promotion.get("changed_path_allowlist") or []}
    if not allowed:
        raise RuntimeError("Final release source-promotion changed-path allowlist is empty")
    observed = set(changed)
    if observed != allowed:
        missing = sorted(allowed - observed)
        extra = sorted(observed - allowed)
        raise RuntimeError(f"Final promotion changed-path closure mismatch; missing={missing}, extra={extra}")

    runtime_allowed = {
        str(item).replace("\\", "/") for item in promotion.get("runtime_source_change_allowlist") or []
    }
    runtime_observed = {item for item in observed if item.startswith("src/psmatrix/")}
    if runtime_observed != runtime_allowed:
        raise RuntimeError(
            f"Final promotion runtime-source closure mismatch; expected={sorted(runtime_allowed)}, "
            f"observed={sorted(runtime_observed)}"
        )

    final_lock = ROOT / str(promotion.get("final_lock_path") or "")
    final_public = ROOT / str(promotion.get("final_public_key_path") or "")
    if final_lock.is_file():
        raise RuntimeError("Final release preparation must not contain an active final release lock")
    if final_public.is_file():
        raise RuntimeError("Final release preparation must not contain an active final release public authority")

    return {
        "contract": contract,
        "anchor": anchor,
        "changed_paths": sorted(observed),
        "runtime_changed_paths": sorted(runtime_observed),
    }
...
def _ensure_output_outside_source(output_root: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError("Final release staging output must be outside the source checkout")
    output.mkdir(parents=True, exist_ok=True)
    if any(output.iterdir()):
        raise RuntimeError(f"Final release staging output must be empty: {output}")
    return output
...
def _wheel_build(build_tree: Path, wheel_dir: Path, version: str) -> Path:
    ...
    if len(wheels) != 1:
        raise RuntimeError(f"Expected exactly one final release wheel, found {len(wheels)}")
    expected_name = f"psmatrix-{version}-py3-none-any.whl"
    if wheels[0].name != expected_name:
        raise RuntimeError(f"Unexpected final wheel name: {wheels[0].name}; expected {expected_name}")
    return wheels[0]
...
def _copy_verified(source: Path, destination: Path) -> Path:
    ...
    if sha256_file(source) != sha256_file(destination) or source.stat().st_size != destination.stat().st_size:
        raise RuntimeError(f"Final artifact copy verification failed: {source.name}")
    return destination
...
def build(expected_head: str, output_root: Path) -> dict[str, Any]:
    ...
    promotion = _require_promotion_boundary(head)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="psmatrix-final-build-a-") as temp_a, tempfile.TemporaryDirectory(
            prefix="psmatrix-final-build-b-"
        ) as temp_b:
            root_a = Path(temp_a)
            root_b = Path(temp_b)
        ...
        report = {
            ...
            "version": version,
            "release_commit": head,
            "rc4_hardened_source_head": promotion["anchor"],
            "rc4_anchor_is_ancestor": True,
            "source_promotion_contract_sha256": sha256_file(_CONTRACT),
            "changed_paths": promotion["changed_paths"],
            "runtime_changed_paths": promotion["runtime_changed_paths"],
            ...
            "ga_eligible": False,
            "next_required": [
                "Review this exact deterministic 2.0.0 artifact set and freeze a separate final release lock.",
                "Sign only the exact final locked artifact bytes with the protected reviewed release authority.",
                "Rebind authoritative Windows evidence to the signed 2.0.0 release rather than relabelling RC4 evidence.",
                "Invoke the existing Production GA evaluator only after final release and final Windows bindings verify.",
            ],
        }
        report_path = output / f"{name}-windows-authority-final-staging.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a deterministic unsigned PSMatrix 2.0.0 final release candidate")
