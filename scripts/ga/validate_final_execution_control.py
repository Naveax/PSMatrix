...
EXPECTED_GATES = ["validation-summary","signed-release","authoritative-windows","complete-runtime-matrix","public-oauth","public-mtls","external-otlp","key-rotation","disaster-recovery","security-review","vulnerability-scan"]
EXPECTED_AUTHORITY_ROLES = {"release","ci","windows-lab","deployment","operations","recovery","security-review","vulnerability-scanner"}
EXPECTED_ENVIRONMENTS = {"production-ga-release-signing","production-ga-windows-lab","production-ga-ci-signing","production-ga-full-matrix","production-ga-public-auth-probe","production-ga-deployment-signing","production-ga-external-otlp-probe","production-ga-operations-signing","production-ga-recovery-signing","production-ga-security-review-signing","production-ga-vulnerability-scanner-signing","production-ga-root-signing"}
...
def validate(root: Path) -> dict[str, Any]:
    root = root.resolve()
    contract = _read_json(root / "ga-packs" / "03-authoritative-windows" / "final-execution-control-contract.json")
    evaluator = _read_json(root / "ga-packs" / "03-authoritative-windows" / "final-ga-evaluator-control-contract.json")
    ...
    controls = contract.get("control_workflows") or {}
    readiness_control = controls.get("readiness") or {}
    evaluator_control = controls.get("evaluator") or {}
    ...
    if {str(item.get("id") or "") for item in auxiliaries if isinstance(item, dict)} != {"public-auth-live-probe", "security-review-packet"}:
    ...
    for key in ("readiness_source_preflight_observed_success","production_readiness_executed","production_evidence_runs_complete","production_evaluator_ready","final_ga_evaluator_invoked","final_ga_attestation_verified","ga_eligible"):
