...
READINESS_HEAD = "18eb9a7eed6fef807e709ce4377fd2709429066d"
...
class FinalExecutionControlContractTests(unittest.TestCase):
    ...
    def test_execution_requirements_are_fail_closed(self) -> None:
        ...
        for key in ("readiness_must_pass_before_production_evidence","readiness_source_preflight_success_required_before_promotion","all_evidence_runs_must_be_workflow_dispatch","all_evidence_runs_must_be_completed_successfully","all_evidence_runs_must_share_exact_execution_control_head","all_evidence_run_ids_must_be_distinct","exactly_one_nonexpired_expected_artifact_per_gate","producer_workflow_source_must_exist_at_execution_head","ga_eligibility_requires_verified_final_attestation"):
    ...
    def test_source_layer_cannot_claim_production_completion(self) -> None:
        ...
        self.assertEqual(set(self.contract["control_source"]["changed_path_allowlist"]), {".github/workflows/ga-final-execution-control-source-preflight.yml","ga-packs/03-authoritative-windows/final-execution-control-contract.json","scripts/ga/validate_final_execution_control.py","tests/test_final_execution_control_contract.py"})

    def test_repository_validator_accepts_exact_current_source_closure(self) -> None:
        result = self.validator.validate(ROOT)
        self.assertEqual(result["status"], "PASS")
        self.assertEqual(result["readiness_source_head"], READINESS_HEAD)
