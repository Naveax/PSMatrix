...
def _service_fixture(root: Path):
...
class WorkerResultCachePathSecurityTests(unittest.TestCase):
    ...
    def test_cached_result_is_revalidated_before_read(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            service, executor, request, worker_private, worker_public = _service_fixture(root)
    ...
    def test_process_revalidates_results_directory_before_cache_write(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            service, executor, request, _worker_private, _worker_public = _service_fixture(root)
            original_lstat = Path.lstat
